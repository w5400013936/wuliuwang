// pages/line-manage/line-manage.js
...
Page({
  ...
  onLoad: function(options) {
    ...
    that.request()
    WxNotificationCenter.addNotification(getApp().shuaXin.faBuZhuanXian, that.guanBi, that)
  },
   ...
   */
  onUnload: function() {
    WxNotificationCenter.removeNotification(getApp().shuaXin.faBuZhuanXian, this)
  },
  guanBi() {
    wx.navigateBack({
      delta: 1
      ...
      success: function(res) {
        ...
        if (res.data.state == 1) {
          ...
          for (var i = 0; i < list.length; i++) {
            ...
            list[i].xianLuNum = list[i].toCitys.split(",").length
          }
          that.setData({
            isfail: false,
            adapter: list,
  ...
  bianJiTap(e) {
    wx.navigateTo({
      url: '/pages/add-line/add-line?bean=' + JSON.stringify(this.data.userInfo1) + "&id=" + JSON.stringify(e.currentTarget.dataset.item.ID),
    })
  },
  ...
  addNet() {
    wx.navigateTo({
      url: '/pages/add-line/add-line?bean=' + JSON.stringify(this.data.userInfo1),
    })
  },
